...
public interface StudentRepository extends JpaRepository<Student, String> {
    // 쿼리 메서드: 메서드 이름에 특별한 규칙을 적용하면 SQL이 규칙에 맞게 자동 생성
    ...
    List<Student> findCityAndMajor(String city, String major);
...
//    @Query("SELECT s FROM Student s WHERE s.city = :cc")
    ...
    // JPQL로 삽입, 수정, 삭제 쿼리 쓰는 법
    @Modifying   // SELECT가 아닌 경우 무조건 붙이기
